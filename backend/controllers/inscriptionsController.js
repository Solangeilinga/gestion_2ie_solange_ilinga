...
exports.getStats = (req, res) => {
  db.query('SELECT COUNT(*) AS total FROM inscriptions WHERE deleted_at IS NULL', (err, total) => {
    if (err) return res.status(500).json({ message: 'Erreur serveur', erreur: err.message });
    db.query(
      `SELECT d.libelle, COUNT(*) AS count
       ...
       JOIN decisions d ON d.id = i.decisions_id
       WHERE i.deleted_at IS NULL
       GROUP BY d.id, d.libelle ORDER BY count DESC`,
      (err2, parDecision) => {
        if (err2) return res.status(500).json({ message: 'Erreur serveur', erreur: err2.message });
        res.json({ total: total[0].total, par_decision: parDecision });
      }
    );
  });
};
